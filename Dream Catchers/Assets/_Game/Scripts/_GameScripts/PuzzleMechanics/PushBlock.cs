...
public class PushBlock : MonoBehaviour {
    ...
    void OnCollisionEnter(Collision collision)
    {
        ...
        if (collision.gameObject.tag == "Player" && !DOTween.IsTweening(transform))
        {
            // The direction the character is facing when colliding with the box
            Vector3 pushDir = collision.gameObject.GetComponent<PlayerMachine>().moveDirection.normalized;
